...
// Assets.hpp and Project/AssetId.hpp are deliberately NOT included: nothing
// here touches the Assets facade. Services::assets survives as an unused
// member -- see SpriteMaterialCache.hpp -- which needs only the forward
// declaration the header already carries.
...
namespace Arcane
{
    ...
    void SpriteMaterialCache::Impl::Bind(Pending& p, Batcher2D& batcher)
    {
        ...
        // THIS FUNCTION NEEDS NO GRAPHICS DEVICE. It compiles no shader
        // OBJECT -- the BLOBS below are what the render recorder builds its
        // own pipeline from.
        ...
        desc.templ = p.templ;
        desc.instance = inst;
        // The retained blobs the render recorder builds its pipeline from
        // (Material2DDesc::vsBytes). Moved out of the
        ...
        // THE DECLARED TEXTURE PARAMS ARE NOT RESOLVED HERE, deliberately:
        // the render recorder reads the declarations straight off `instance`
        // (ResolveTextures() -> Guids) and resolves them through
        // NriTextureCache onto its own device. The Guids are the durable form;
        // a GPU object resolved here would belong to the wrong owner.
