...
std::unique_ptr<GpuContext> GpuContext::Create(const LoomConfig& cfg)
{
    ...
    wd.vulkan = (cfg.backend == Arcane::GraphicsBackend::Vulkan);
    ...
    ctx->m_device = Arcane::RenderDevice::Create(dd);
    ...
    ctx->m_shaders = Arcane::ShaderLibrary::Create(ctx->m_device->Nvrhi(), cfg.backend, "shaders");
    if (!ctx->m_shaders) { ARC_ERROR("GpuContext: shader library create failed"); return nullptr; }

    ctx->m_canvas = Arcane::CreateCanvas(ctx->m_device->Nvrhi(),
                                         ctx->m_swapchain->Width(),
                                         ctx->m_swapchain->Height());
    if (!ctx->m_canvas) { ARC_ERROR("GpuContext: canvas create failed"); return nullptr; }

    ctx->m_batcher = Arcane::Batcher2D::Create(ctx->m_device->Nvrhi(), *ctx->m_shaders);
    if (!ctx->m_batcher) { ARC_ERROR("GpuContext: batcher create failed"); return nullptr; }

    ctx->m_tonemap = Arcane::TonemapPass::Create(ctx->m_device->Nvrhi(), *ctx->m_shaders);
    ...
    ctx->m_imgui = Arcane::ImGuiLayer::Create(ctx->m_window, *ctx->m_device, *ctx->m_shaders);
