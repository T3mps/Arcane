...
namespace Arcane::Editor
{
    ...
    void ShaderEditorDocument::DrawSnippetEditor(float height)
    {
        ...
        if (m_editVertex)
            ImGui::TextDisabled(IsGraphOwned()
                                    ? "vertex stage (GENERATED by the base graph's "
                                      "Vertex Output node)"
                                    : "editing: vertex stage (Varyings "
                                      "displace(Varyings v); empty = passthrough)");
        else if (ChainMode())
        ...
        // but never hand-edited. Same for the vertex stage when the BASE is
        // graph-owned (its Vertex Output node generates the body); text-owned
        // materials hand-author it.
