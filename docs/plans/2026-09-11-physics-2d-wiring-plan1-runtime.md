...
Plan 1 is closed at this document's HEAD range **`f2eeddfa..6400581c`** (`6400581c` is Task 11's baselines-booking commit; the closeout-notes commit for this section follows immediately after). Gacha (`D:\dev\starworks\Gacha`) is at `b860e9e0`, its own Task 10 restamp.

**State handed off:**
- Engine ABI is **28**.
- Physics is **engine-owned and always on** — no enable flag, no game-module include surface (`build/arcane.lua` carries no Manifold2D row; only the editor and `ArcaneTests` link it). `Runtime::InstallEngineSystems()` installs `PhysicsSystem` into `fixedUpdate` from the ctor and every `ClearSystems()` (every `PluginHost` reload path); `Runtime::EnsurePhysics()` mints the world if absent (scene open, `RestoreRegistry`, hot reload) and re-mints on a gravity change (the vendored `PhysicsWorld` has no `SetGravity`).
- `PhysicsSettings{gravity}` is a reflected, roster-registered scene-root component; `.arcproj` carries a `"physics": {gravity}` project-default block (`ProjectManifest::PhysicsConfig`, lenient parse — a malformed value keeps the default rather than failing the manifest). `Runtime::ResolvedGravity()` reads the scene-root override over the project default.
- `PhysicsSystem`'s paused-pass rules are fixed: PASS 1 re-mints only on `Changed<Collider2D>`/`Changed<RigidBody2D>` while paused; PASS 1.5 auto-adds the structure-locked `PhysicsBodyRef` the Inspector can never add; PASS 4 (pose write-back) is gated on `m_stepWorld`, so Edit mode never moves a body.
- `EditModeSchedule::SetPhysicsEditPass` runs the edit pass before propagation every Edit-mode frame, never in Play; `EditorApp` binds it to `Runtime::PhysicsEditPass()` where `m_runtime` is constructed. Play/Stop returns bodies to their authored poses through a fresh world (`Runtime::RestoreRegistry` now strips `PhysicsResource`/`PhysicsInterpBuffer` after `Astra::Registry::Load` — Task 6's real bug fix, below).
- The editor has a **Physics Overlay**: `PhysicsDebugDrawOptions::onlyBody` filters to one entity's outline with no other overlay noise; the View menu's "Physics Overlay" toggle (`m_physicsOverlay`) draws the whole world's debug geometry; `ArcaneEditor.exe` does **not** link `Manifold2D.lib` (the selection's body handle resolves through `PhysicsResource::entityToBody`, a plain Arcane-level map, not a direct `PhysicsWorld` method call).
- The reflection→JSON bridge grew a **container branch** (vectors of reflected structs only — scalar-element vectors remain unsupported by design); `Collider2D::fixtures` is serializable again; scene schema is **v5** (v4 files still load).
- `ReferenceProject/Content/scenes/physics.arcscene` (guid `4f6a1c2e-7b3d-4e8a-9c1f-2d5b6e7a8f90`, **not** the boot scene) is a real content fixture: a static `Ground`, three dynamic bodies (`Crate`/`Ball`/`Pill` — Aabb/Circle/Capsule), scene-root `PhysicsSettings`. The **W4** witness (`WitnessScenariosTest.cpp`, `[witness][gpu]`) proves a dynamic body authored off the ground falls and lands under the real runtime host, observed from outside.
- Gacha's `Game/Aphelyon.arcproj` is restamped to ABI 28 and `Aphelyon.dll` rebuilt against this SDK (Task 10, `b860e9e0`); Gacha's own scenes carry no physics content by the user's earlier call — the block and component roster are there for when they do.
- The golden lanes needed a re-bless mid-close (not "untouched," unlike Plan 1's own line-30 premise for the *scene*): Task 8's editor View menu is part of the editor's chrome, drawn regardless of scene, and it shifted the `editor-ui` golden's top-menu-bar pixels. `golden-gate.ps1 -Configuration Release` now reports **4/4 lanes, diffCount=0** after the re-bless (`50df63b2`).

**Final derived counts (Task 11's own runs, both configs agreeing):**
- Debug unfiltered (`ArcaneTests.exe`, all tests incl. `[witness][gpu]`): **118692 assertions / 1689 test cases**, all passing, seed 1842615517 — proves the freshly rebuilt `ReferenceProject/Binaries/ReferenceGame.dll` is current for both Debug and Release `Arcane.slnx` builds and that W4 runs clean under the full suite.
- Debug `~[gpu]`: **56399 assertions / 1655 test cases**, all passing, seed 3897406862 (console run); the `-r json` run fed to the guard measured the same 56399/1655.
- Release `~[gpu]`: **56399 assertions / 1655 test cases**, all passing, seed 136951388 (console run); the `-r json` run fed to the guard measured the same 56399/1655 — **Debug and Release agree exactly.**
...
- **Attribution against the pre-plan baseline (56216/1632):**

  | Task | Δ assertions | Δ cases | What moved |
  |---|---|---|---|
  | T2 (`edd0bb9e`) | +8 | +1 | `VendorSmokeTest`'s new case (8 `CHECK`s) — Astra `FieldInfo` vector-element access vendored |
  | T3 (`cdf4ba1e`) | +64 | +3 | 3 new cases: `"a vector of reflected structs round-trips as a JSON array"` (44), `"scene round-trips Collider2D fixtures through JSON"` (15), `"a v4 scene still loads after the v5 bump"` (5); a 4th `SceneJsonTest` hunk only renamed a pre-existing case (body/count unchanged) |
  | T4 (`590c4bcc`) | +24 | +4 | 4 new cases summing to 22 (`"a manifest physics block sets gravity..."` 5, `"a malformed physics gravity..."` 6, `"PhysicsSettings is a reflected, roster-registered scene component"` 7, `"PhysicsSettings on the scene root round-trips through JSON"` 4) — **2 assertions unresolved**, see `automation-baselines.json`'s note for this close (commit `590c4bcc` is purely additive, 0 deletions; the residual is named, not smoothed over) |
  | T5 (`69a62186`) | +25 | +6 | 6 new `[transform-sync]` cases in `AuthoredTransformSyncTest.cpp`, summing exactly |
  | T6 (`3ae99971`) | +33 | +5 | 5 new `[runtime][physics]` cases in `RuntimeTest.cpp` (32) + a 6th change that renamed a pre-existing case with a net +1 assertion (33 total) |
  | T7 (`7a7c9cd8`) | +19 | +2 | 2 new `[editor][physics]` cases, summing exactly |
  | T8 (`ef0f5655`) | +10 | +2 | 2 new overlay cases, summing exactly |
  | T9 (`861c19b5`) | +19 | +1 | W4, `[witness][gpu]`-tagged — **invisible to `~[gpu]` by design**, counted in the unfiltered total only |
  | **Net (`~[gpu]`)** | **+183** | **+23** | matches this task's own measured 56399/1655 exactly |

**Baseline guard outcome:** `check-baselines.ps1` against the still-committed `56216/1632` reported the rise on both metrics before this task's edit; booked into `scripts/automation-baselines.json` (`6400581c`) with the per-task/per-case attribution above transcribed into that file's own dated note; the guard re-run against the updated file reported **`+0`/`+0`, exit 0** on both Debug and Release.

**Rulings the controller made during this plan** (full detail in `.superpowers/sdd/2026-09-11-physics-2d-wiring-plan1-runtime/progress.md`):
- Budget mode throughout: controller on Sonnet, inline `executing-plans`, no subagents, no per-task reviews — checkpoints after T2, T6, T9, T11 only.
- T5's two new PASS 1 views (`Changed<Collider2D>` gated only by `With<RigidBody2D>`, and vice versa) failed Astra's `ChangeTermsAreRequired` static assert — current Astra requires a `Changed<T>` type to also be a required fetch component, not just named in a `Changed<>` term. Fixed by adding `const Collider2D`/`const RigidBody2D` to each view's fetch list.
- T6 found and fixed a **real product bug**, not a plan-snippet defect: Astra's vendored `ResourceStorage::Serialize`/`Deserialize` now serializes every resource offering `Serialize(Archive&)` unconditionally (no opt-out), so `Runtime::RestoreRegistry` was reviving a default-constructed, world-less "zombie" `PhysicsResource` after every Play→Stop or snapshot restore — silent, order-dependent flakiness. Fixed by explicitly stripping `PhysicsResource`/`PhysicsInterpBuffer` from the loaded registry right after `Astra::Registry::Load`, before the swap. This pattern generalizes: any future transient/host-owned `Runtime` resource with a no-op `Serialize` will silently ride along through `RestoreRegistry` unless stripped the same way — no general opt-out exists for resources (only reflected components have `Serializable(false)`).
- T6 and T7 each fixed one plan-snippet test defect comparing a pointer to just-freed memory against a fresh allocation after a world re-mint (allocator reuse legitimately produces the same address) — removed the address comparison, kept the `REQUIRE(!=nullptr)` that is the real invariant.
- T7 fixed a plan-snippet test defect comparing a fixed `before` baseline across multiple `RunFrame` calls against propagation's monotonically-increasing run count — fixed with a rolling `lastRuns` updated after each call.
- T8: `ArcaneEditor.exe` does **not** link `Manifold2D.lib` — the plan's snippet called `phys->world->IsValid(ref->handle)` directly, an out-of-line `PhysicsWorld` method, unresolved at link time. Fixed by resolving the selection's body handle through `PhysicsResource::entityToBody` instead.
- T9 Step 2: the plan's `ArcaneEditor.exe --scene <guid> ...` load-proof command does not run — `--scene` is not implemented on the editor host (it always boots the manifest's `bootScene`; the flag lives on `ArcaneRuntime.exe` only). Substituted the runtime host — exit 0, log confirms the scene loaded and the physics world minted, screenshot shows all four sprites already at rest (stronger evidence than the plan's intended Edit-mode "nothing moves" proof, just via a different host).
- T9 Step 3: a **real defect in the plan's test snippet**, caught by a process crash rather than a failed assertion — the plan's W4 put two `pick@x,y` probes in one run, but only the FIRST `pick@x,y` probe in any run is ever armed (`VerifyReport.cpp`); the second gets an `"error"` entry, never `"entity"`. Indexing that through a `const nlohmann::json&`'s `operator[]` hit `JSON_ASSERT` on the missing key (exit 3), not a Catch2 `FAILED`. Fixed by splitting W4 into two separate `WitnessScratch`/`RunWitness` invocations, one probe each.
- T11 Step 2: the plan's line-30 premise ("no render change ⇒ the four golden lanes are untouched, no re-bless") held for the physics demo scene but not for Task 8's editor View menu, which is part of the editor's own chrome and shifted the `editor-ui` golden regardless of scene content. Re-blessed on dx12 (the shared reference slot) per the established procedure, restaged to both Debug and Release copies of both hosts, committed separately (`50df63b2`) with the reasoning inline.

**Deferred / not actioned, each with its owner:**
- `SetGravity` upstream in Manifold2D (`D:\dev\starworks\Manifold2D`) — until it lands, a gravity edit re-mints the whole world rather than pushing the new value.
- Scalar-element vectors in the reflection→JSON bridge — no roster field needs one yet; the bridge handles reflected-struct elements only and fails loud otherwise.
- The α=0.25 blend-direction pin — deferred to Plan 2 with the Inspector-era integration case it belongs to.
- **Plan 2** (spec §7.3): the Inspector `FieldKind::Vector` editor. Until it lands, fixture lists (`Collider2D::fixtures`) are authored by hand in `.arcscene` JSON, as `physics.arcscene` was here.
- The 2-assertion residual in Task 4's attribution (above) — named in both this section and `automation-baselines.json`'s note, not resolved; the total (56399/1655) is independently triple-measured this close and is not in doubt, only the last mile of its per-case decomposition.
