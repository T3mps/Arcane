...
**Interfaces:**
- Produces: `namespace Arcane::Diag` -- `struct Envelope { std::uint32_t formatVersion = 1; Guid guid; std::string kind, timestampUtc, appName, phase, buildInfo; std::string cpuThreadSummary; struct Queue { std::string name, lastCompleted; std::vector<std::string> inFlight; }; std::vector<Queue> queues; struct Fault { std::string type, address, resource; } fault; std::string backendBlobPath; std::string siblingTxt, siblingDmp; std::vector<std::string> activeLayers; };` and `std::string Serialize(const Envelope&)`, `std::optional<Envelope> Parse(std::string_view json)`, `bool WriteFile(const Envelope&, const std::filesystem::path&)`, `std::optional<Envelope> ReadFile(const std::filesystem::path&)`.
