...
- vcpkg install (`libpq:x64-windows-static-md --overlay-triplets=<repo>/vcpkg-triplets --no-binarycaching`, run by the orchestrator in the background, no parallel Server build): **exit=0**, "All requested installations completed successfully in: 12 min" (openssl 8.1 min, zlib 8.3 s, libpq 3.4 min) (per-package figures from the orchestrator's own vcpkg install log — its `Elapsed time to handle <pkg>:x64-windows-static-md` lines; the Task 2 report carries only the 12-minute total).
...
**Q3 consequence (fact, established by Task 4's Release suite run above).** The Server's Release configuration defines `NDEBUG` as of Task 3; the three services' Release refusal guards are live for the first time. The full Release-only list (seven further sites) is recorded in ruling Q3's final-review correction above; the Release services now require `APHELYON_INTERNAL_SECRET`, `APHELYON_DB_CONNECTION` (Account) and `APHELYON_QUEST_TOKEN_SECRET` (Account) to boot. Not measured by this plan: a Release end-to-end boot (`start-all.bat release`) exercising the three `Stop()`-on-probe-failure paths — owed as a desk item. Also from the final review: the fix-round commit `bb414f79` added `disablewarnings { "4251" }` to `arcane_game_module()` as well as `arcane_core_consumer()` (no header change, so no Game rebuild or ABI restamp; the next `arcbuild` simply emits fewer warnings); `Common.lib` absorbs `ArcaneCore.lib`'s import members because premake routes a StaticLib's `links` into `<Lib>` — harmless (every exe links the import lib itself), a link-less helper variant for StaticLib callers is an SDK follow-up; `Cli::PrintUsage` renders long names only, so `--help` lost the `-p/-v/-q/-u` short-alias display the old usage text had — an engine follow-up.

**Final review + fix wave.** Verdict: **With fixes.** The three Importants: (1) the Gacha `CLAUDE.md` sentences describing the quest-token secret and the DB connection's Debug fallback as unconditional needed the Release-refuses-without-it correction (ruling Q3's ten-site NDEBUG blast radius, not three); (2) the `/arch:AVX2` baseline this plan imposes on every Server project was undocumented as an operational prerequisite (`Server/BUILD.md`, `scripts/doctor.bat`); (3) `scripts/doctor.bat` and `ci/README.md` did not check or state that `ARCANE_SDK` must be a BUILT checkout, the exact failure mode the Jenkins `Generate` stage now hard-refuses on. Commits: Gacha `080cbb93` — Arcane = this commit.
