...
- **Suites run FROM the exe dir, FOREGROUND:** `cd bin/Debug-windows-x86_64-md/ArcaneTests && ./ArcaneTests.exe "<filter>" | grep -E "^Randomness|^test cases|^assertions|All tests passed|FAILED"` — never `cat` a suite log. The whole-suite filter is `~[gpu]` (last booked baseline: 57562 assertions / 1788 cases, Debug and Release, 2026-09-16 — DERIVE the new numbers, never recall). Record every seed.
