...
1. **Combat grid Mesh path → IMMEDIATE-MODE WINS (resolved 2026-05-28).** CP-3 baseline of the combat scene showed steady-state `scene.drawcalls = 3` — LÖVE 11's auto-batcher was already coalescing the entire 10×10 polygon storm into a handful of draw calls. A MeshBatch refactor regressed to ~26 draw calls because the explicit `love.graphics.draw(mesh)` calls break the auto-batch chain. Per this lever's pre-flagged mitigation, the combat-grid POC is **dropped**; combat keeps the immediate-mode `polygon("fill")` stream which the auto-batcher handles optimally. The MeshBatch primitive itself (T3) remains valuable for WaveGame's 1500-poly storm where per-frame state changes prevent auto-batching. **Durable lesson:** profile BEFORE assuming the audit's qualitative numbers describe the auto-batcher's blind spot. `setColor` doesn't break the batcher; only shader/texture/blend/canvas switches do — and the combat grid uses none of those per tile.

2. **WaveGame tunnel → DEFERRED PENDING INTERACTIVE PROFILING (2026-05-28).** `--scene dailies` pushes the Dailies screen, but WaveGame's playback is gated behind a button-press; the inner `_drawTunnel` only runs once playback starts, which the headless `--profile-capture` CLI can't trigger. Baseline capture showed Dailies idle (`scene.drawcalls = 5`), not WaveGame playback. The structural argument from the audit (2 explicit `setBlendMode` switches per pass DO break the auto-batcher, unlike combat's pure-`setColor` stream) still favors a MeshBatch refactor — but shipping unvalidated changes is risky. **Action:** defer T6 until either (a) the CLI gains a `--play-wave` flag that simulates the button press, or (b) we capture a manual interactive playback session. The MeshBatch primitive's existence is justified by T12 (Panel chrome) regardless.

3. **Atlas bake hangs boot when populated (2026-05-28, infra-only ship).** T8 wired the runtime atlas pipeline: `services/assets/atlas.lua` (skyline-pack + blit), `Assets.bakeAtlases()` called from `love.load`, `Assets.image(key)` returns `(tex, quad)`, `Assets.draw(key, ...)` helper, the atlas-aware `ui/widgets/image.lua`. **But:** when `data/atlases/ui.json` declared 216 keyboard/mouse icon keys, the engine hung mid-bake — boot never reached the main loop, no CSV got written, no Lua error surfaced (suggesting a native-side issue in the blit-to-mipmap-canvas path on the 2048² target). The bake is `pcall`-wrapped now so a future repopulation can't re-crash, but the underlying hang isn't diagnosed. **Action:** ship T8 with `ui.json keys: []` (no-op bake on boot). The `Assets.image` two-value return shape + `Assets.draw` helper + skyline + atlas builder are all in place; populating an atlas becomes a follow-up once the hang is isolated. Suspects to investigate: (a) `newCanvas(2048, 2048, {mipmaps="manual"})` — try `mipmaps="none"` or omit; (b) `setBlendMode("replace", "premultiplied")` — try the default; (c) per-image `newQuad` allocation cost at 200+ items.
