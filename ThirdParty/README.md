...
| **bc7enc_rdo** | `b943862` (b9438627eef73a1157e84201b6fa6eb2ffd6d9f0) | MIT or public domain, dual-licensed | Arcane | BC7 block encoder (`bc7enc.*`) + decoder (`bc7decomp.*`) for `ArcaneAssetPipeline`'s texture cook (F2b Task 4). Vendored slice only -- excludes the RDO post-process (`ert.*`, `rdo_bc_encoder.*`), `rgbcx.*` (BC1-5), the ISPC kernel, lodepng, and the example CLI/test images; plain `bc7enc_compress_block` with pinned params is deterministic by construction (no thread/RNG surface in the vendored files) | https://github.com/richgel999/bc7enc_rdo |
| **Catch2** | v3 | Boost 1.0 | Server, Arcane | Unit test framework (reducer tests, integration tests) | https://github.com/catchorg/Catch2 |
| **cgltf** | `bbeb5b0` (bbeb5b0b070ddacddac6852fb72143eb68454937) | MIT | Arcane | glTF 2.0 loader for `ArcaneAssetPipeline`'s mesh cook (F2c Task 1). 1.15 + the CVE-2026-32845 sparse-accessor overflow fix from jkuhlmann/cgltf#293, pinned by commit -- `cgltf_validate` is mandatory on every import, spec §4.5 (PR unmerged upstream; applied by hand, recorded in `ThirdParty/cgltf/PATCHES.md`). Header-only; one implementation TU (`ArcaneAssetPipeline`'s `CgltfImpl.cpp`). `cgltf_write.h` deliberately not vendored -- nothing in Arcane writes glTF | https://github.com/jkuhlmann/cgltf |
...
| **meshoptimizer** | `9d9890c` (9d9890c73011d75920af614485296d1e03e95448) | MIT | Arcane | Mesh optimization (vertex-cache/overdraw/fetch, simplification, `EXT_meshopt_compression` codec, tangent generation) for `ArcaneAssetPipeline`'s mesh cook (F2c Task 1). Whole upstream `src/` vendored, not file-level curated -- see `ThirdParty/meshoptimizer/premake5.lua`'s header for why (simplification/compression-decode/tangent-gen are vendored-but-dormant until later F2c tasks switch them on); tooling excluded (`gltf/`, `js/`, `demo/`, `tools/`) | https://github.com/zeux/meshoptimizer |
