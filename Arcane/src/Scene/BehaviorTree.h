...
namespace ARC::BehaviorTree
{
   ...
   { 
   public:
      static const int32_t INFINITE_COUNT = -1;

      Repeater(std::shared_ptr<Node> child, int32_t repeatCount = INFINITE_COUNT) : Decorator(child), m_repeatCount(repeatCount) {}

      Status Update(Scene& scene, Entity entity, Blackboard& blackboard) override;

   private:
      int32_t m_repeatCount;     // -1 for infinite repetition
   };
   ...
   {
   public:
      Limiter(std::shared_ptr<Node> child, int32_t maxCalls) : Decorator(child), m_maxCalls(maxCalls), m_callCount(0) {}
