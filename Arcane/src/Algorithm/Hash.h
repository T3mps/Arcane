...
namespace ARC::Hash
{
   ...
   constexpr uint64_t RotateLeft64(uint64_t value, int32_t shift) { return (value << shift) | (value >> (64 - shift)); }
   ...
   inline uint64_t xxHash64Runtime(const std::string& data, uint64_t seed = 0) { return xxHash64(data.data(), data.size(), seed); }
