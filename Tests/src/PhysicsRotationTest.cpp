...
// ============================================================================
// (b) Rotated manifold normal: a box tilted at ~30° resting on a flat floor.
//
// The contact normal generated by Collide must reflect the actual face normal
// of the contact geometry (the box face nearest the floor), NOT always (0,−1).
//
// With a moderate rotation the narrowphase should pick the downward-facing
// box edge as the reference or incident face and produce a normal that differs
// from (0,±1) by at most a small deviation OR the floor-face normal (0,−1).
// Specifically: the manifold normal dot with (0,1) > 0.9 within 1e-2.
// (Any contact that correctly resolves the overlap must point roughly upward
// in world space, though the exact normal depends on which face is reference.)
// ============================================================================
TEST_CASE("physics-v2 T5 (b): rotated box manifold normal is valid contact direction",
          "[physics]")
{
          ...
          "[physics]")
{
    ...
    // Second fixture: circle r=6 offset 18 units to the right.
    // With the body at x=0, the circle's world center is at (18, body_y).
    // When the box bottom (y + 10) reaches the floor (y=200), the circle
    // (radius 6) also has its bottom at (y + 6) which is ABOVE the floor,
    // so the box lands first. After landing, we only check that both fixture
    // contacts were generated (body didn't tunnel; the circle center extends
    // to a position where it can also contact the floor if it were lower).
    //
    // For the compound detection test: use a bigger circle that is ALSO above
    // the body center, ensuring it also contacts the floor when the box does.
    // Actually: fixture at local offset (18, 0) with r=6 -> bottom at y+6
    // when box bottom is at y+10 -> circle doesn't reach floor when box lands.
    //
    // REVISED: place the circle below the box center so both contact the floor:
    // circle at local (18, 4) with r=6 -> bottom at body_y + 4 + 6 = body_y + 10
    // (same as box bottom). Both fixtures touch the floor simultaneously.
    FixtureDef fd;
    ...
    // The key detection assertion: with TWO fixture contacts being generated,
    // the solver had enough constraint pairs to resolve both the box-floor
    // and circle-floor overlaps. A body that passes through the floor would
    // have finalPos.y >> 200, which the first two CHECKs already catch.
    // The second fixture (circle at offset) generates its OWN contact constraint
    // which the solver uses -- this is the T5 fixture-pair contact gen gate.
    //
    // We verify the circle fixture also has a world position above the floor
    // (not sunken through it) -- demonstrating that the solver resolved both
    // fixture contacts correctly.
    ...
    // The circle bottom should be AT or ABOVE the floor surface (not tunneled).
